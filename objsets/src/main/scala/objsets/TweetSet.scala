...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    lazy val lefts = leftmost.retweets
    lazy val rights = rightmost.retweets
    val count = elem.retweets

    (left, right) match {
      case (_:Empty, _:Empty) => elem
      case (_:NonEmpty, _:Empty) => if ( lefts > count ) leftmost else elem
      case (_:Empty, _:NonEmpty) => if ( rights > count ) rightmost else elem
      case _ => if ( count > lefts && count > rights ) elem else if ( lefts > rights ) leftmost else rightmost
    }
  }
